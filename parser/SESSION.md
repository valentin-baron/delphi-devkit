...
| 43 | Non-`Defined/Declared/SizeOf` function call in a `{$IF}` condition | CLOSED (2026-08-03). System.pas (D12) guards a body with `{$IF Length(RegisteredTypeInfoTable) = 1}`. The condition evaluator (`if_eval.rs`) only special-cased `Defined`/`Declared`/`SizeOf`; a bare identifier `Length` was read as a const (→Unknown) and the trailing `(...)` then hit `expect_eof` → `EvalError::Syntax { "unexpected trailing tokens" }`. Unlike Unknown (tolerated via the AssumeFalse/True policy + Warning), an `EvalError` surfaces as a FATAL `CursorError::Condition` that fails the whole unit — so System.pas (and any other unit using a value-function in a directive) was silently dropped from the RTL bootstrap. Fix: `ident_factor` now recognises ANY identifier followed by `(` as a function call, consumes a balanced comma-separated argument list (`consume_call_arguments`, reusing `expression()` so nested calls/parens are handled), and returns `None` (Unknown) — we cannot evaluate it, so per never-wrong we degrade to Unknown, never a guess and never a hard failure. Added a `Comma` token to the evaluator lexer (multi-arg calls like `Max(a,b)` now tokenise; a stray comma outside a call is still a Syntax error). An unterminated call (`Length(X = 1`) is still a Syntax error. Regression: `if_eval::tests::general_function_call_is_unknown_not_error`. FOLLOW-UP (precision, not correctness): a curated set of purely-constant functions (`Ord`, `Odd`, `Length` of a known const array, `High`/`Low` of a builtin) could be evaluated to True/False instead of Unknown — deferred; Unknown is safe (a body-level `{$IF}` only affects the over-approximating usage index). |
