...
- **Structured diagnostics in `ddk compile --json`**: the JSON output now carries a `diagnostics` object next to the existing `lines[]`, grouped into `errors`/`warnings`/`hints`, with each entry holding `{code, file, line, message}` (absolute file path, numeric line). Machine consumers can read code/file/line directly instead of regexing formatted strings. It obeys the same `--show-warnings`/`--show-hints` filters as `lines[]` (errors always present; warnings/hints only with their flag), so a slimmed compile stays slim in both views. `lines[]` is unchanged (backwards compatible).
